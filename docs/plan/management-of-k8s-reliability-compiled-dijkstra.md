...
| Slice 2 — Call context API redesign | Not started | Largest slice. New `src/cache/PartitionRef.ts`; [CallState.ts](../../src/call/CallState.ts) API extension; [SipRouter.ts](../../src/sip/SipRouter.ts) cookie extraction at event ingress; [PartitionedRelayStorage.ts](../../src/cache/PartitionedRelayStorage.ts) reverse-propagate write path; [AtomicWriter.ts](../../src/replication/AtomicWriter.ts) + Lua `direction` tag; [ReplLog.ts](../../src/replication/ReplLog.ts) / [ReplPuller.ts](../../src/replication/ReplPuller.ts) / [ReadyGate.ts](../../src/replication/ReadyGate.ts) direction-aware. |
