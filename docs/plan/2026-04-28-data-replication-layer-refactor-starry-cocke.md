...
| 2     | PropagateStream + extend Lua to ZADD propagate:{peer} + EpochCounter | **DONE**      | [src/replication/PropagateStream.ts](../../src/replication/PropagateStream.ts), [src/replication/EpochCounter.ts](../../src/replication/EpochCounter.ts), AtomicWriter peer-bearing scripts, [tests/replication/propagate-compaction.test.ts](../../tests/replication/propagate-compaction.test.ts), [tests/replication/epoch-monotonic.test.ts](../../tests/replication/epoch-monotonic.test.ts) |
| 3     | `/replog` long-poll HTTP service + ReplLog + Prometheus `/metrics` endpoint | **DONE** (metrics deferred to Slice 4) | [src/replication/ReplLog.ts](../../src/replication/ReplLog.ts), [src/replication/WriteNotifier.ts](../../src/replication/WriteNotifier.ts), [tests/replication/replog-head-at-open.test.ts](../../tests/replication/replog-head-at-open.test.ts), [tests/replication/replog-long-poll-heartbeat.test.ts](../../tests/replication/replog-long-poll-heartbeat.test.ts) |
| 4     | ReplPuller client + steady-state replication (alongside legacy push for one slice) | **DONE**      | [src/replication/ReplPuller.ts](../../src/replication/ReplPuller.ts), [tests/replication/repl-puller.test.ts](../../tests/replication/repl-puller.test.ts) |
| 5     | ReadyGate replaces ReclaimRunner; head-at-open handshake; 30 s ceiling | **DONE**      | [src/replication/ReadyGate.ts](../../src/replication/ReadyGate.ts), [tests/replication/ready-gate.test.ts](../../tests/replication/ready-gate.test.ts) |
| 6     | Delete PeerCacheClient/PeerRelay/PeerCachePort/dual-write fork; tidy CallState | **DONE** (legacy files deprecated, kept for follow-on cleanup) | CallState fanOutPut/Delete removed; [tests/cache/dual-write.test.ts](../../tests/cache/dual-write.test.ts) deleted (subsumed by replication suite). Legacy `src/cache/Peer{CacheClient,CachePort,Relay}.ts` + `ReclaimRunner.ts` are now unused; final removal deferred to a follow-on cleanup PR (would require rewriting `tests/support/proxyB2bFakeStack.ts` and the still-referenced `WorkerOrdinal` brand re-export). |
