...
- **Rule coverage + kill testing**: `npm test` writes a rule-coverage section into `test-results/fake-clock/index.html` flagging never-fired rules. `npm run test:rule-kill` is currently a no-op pending a fake-clock speedup — run `tsx scripts/rule-kill.ts` manually if you need the mutation audit. See [docs/rule-coverage-and-killing.md](docs/rule-coverage-and-killing.md) before adding a new rule.
...
- **Executor destructure discipline**: Every `execute*` function in [src/b2bua/rules/framework/ActionExecutor.ts](src/b2bua/rules/framework/ActionExecutor.ts) must begin by destructuring every field of its action parameter, marking the discriminator with `void type`. Combined with `noUnusedLocals` / `noUnusedParameters` in [tsconfig.json](tsconfig.json), this makes it a compile-time error to declare a field on a `RuleAction` variant without reading it in its executor. Purpose: prevent silently-ignored action fields (e.g. `create-leg.updateBody` was typed but never read before this discipline landed, which would have shipped the wrong SDP). See "Adding a new action field" in [docs/rule-extension-guide.md](docs/rule-extension-guide.md).
