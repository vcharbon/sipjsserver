...
An integrator-authored bundle of cooperating [[rule SDK]] rules implementing one callflow (PRBT, announcement, MRF-during-transfer, blind transfer, …). Its rules are *minted from the service* (`defineService({...}).rule(...)`) so they share — by construction, keyed by the service id over opaque replicated storage — typed **call-ext** (`callExt`, optionally carrying a **phase** machine that gates which rule fires) and optional typed **per-leg ext** (`legExt`). Three properties define the contract:

- **Applicability (the guard / `isApplicable`).** Exactly one predicate `(ctx) => boolean` decides whether the *whole* ruleset applies to a call; individual rules never re-check activation. **It must be `false` unless the service genuinely owns the call** — narrow it to the precise activation condition, never a broad "feature is present" check. Default: the per-service slice (`call.ext[id]`) is present (presence-based activation, seeded by the decision response — superseding the earlier closed feature-flag union); pass `isApplicable` for richer conditions, or `alwaysActive` for core-always services. An over-broad guard makes a service active alongside a sibling that owns the call and leaks its rules into the wrong flow.
- **Precedence by layer + explicit order.** A service's rules live *above* core defaults (`SERVICE_LAYER` > `CORE_LAYER`), so when active they automatically win over a colliding core rule for the same event — the author never names a core rule id. There is **no per-rule priority/specificity**; *within* a layer the **declared order** is authoritative (first-match-wins). `overrides`/`composesWith` remain only as rare layer-agnostic escape hatches (e.g. a core rule that must trump a service rule, or two same-layer rules that must resolve one way).
- **Decline by returning `undefined`.** A rule whose `handle()` returns `undefined`/`void` *declines* the event: it is not consumed, and the dispatcher continues to the next candidate in the same layer and then down to lower layers. Returning an empty `{ actions: [] }` *consumes* it (a no-op handled event); these are different. Use `undefined` to let a more general rule (often a core default) handle the case.

Two active services coexist because every slice is service-id-keyed, but two services cannot co-own the [[active peer]] bridge on one call at the same instant, and two services that both match an event must be mutually exclusive by applicability (or ordered/`overrides`-resolved). `promote18xPemTo200`, the in-tree REFER blind-transfer flow, and the `relayFirst18xTo180` policy module are all callflow services on this template (typed service-keyed slices; the bespoke `Call.transfer` field and `transferPhase` match column are retired). An [[extension]] ships one or more.
