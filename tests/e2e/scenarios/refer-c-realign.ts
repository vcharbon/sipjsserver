 ...
 *
 * These scenarios require `referAllowEnabled: true` on the runner.
 *
 * Scenarios:
...
export const referAllowCRealignHappy = scenario("refer-allow-c-realign-happy", (s) => {
  ...
  // Slice 6 stops before re-INVITE A — the test teardown uses alice.bye()
  // which relays to both confirmed bLegs (bob + charlie). We explicitly
  // script BYE at both sides below.
  ...
  // Teardown — A hangs up; relay-bye fires BYE to b-1 and b-2.
...
export const referAllowCRealignCGlare = scenario(
  ...
  (s) => {
    ...
    // Teardown — A BYE relays to both b legs.
    const aliceByeTxn = aliceDialog.bye()
    const bobByeTxn = bobDialog.expect("BYE")
    bobByeTxn.reply(200)
    const charlieByeTxn = charlieDialog.expect("BYE")
