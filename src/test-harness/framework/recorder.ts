...
export interface ClusterContext {
  ...
  /**
   * Assert that worker `workerId`'s sidecar holds the SIP-derived
   * index `idx:leg:{callId}|{tag}`. Fails if the call body is on
   * the worker but the index is missing — the exact failure mode
   * that drove production's mid-dialog 481 storm. `tag` may be
   * omitted when the test does not pin the From-tag (random-
   * generated by the framework); matches any tag in that case.
   */
  expectIndexOnBackup(
    workerId: string,
    opts: { callId: string; tag?: string; present?: boolean }
  ): void
  /**
   * Assert the cluster-shared CallLimiter holds `expected` inflight
   * count for `limiterId`, summed across the active windows the
   * same way `CallLimiter.checkAndIncrement` reads.
   */
  expectLimiterCount(limiterId: string, expected: number): void
}
...
export function record(
  ...
  const cluster: ClusterContext = {
    ...
    expectIndexOnBackup(workerId, opts) {
      recordK8s({
        kind: "expectIndexOnBackup",
        workerId,
        callId: opts.callId,
    ...
    expectLimiterCount(limiterId, expected) {
      recordK8s({ kind: "expectLimiterCount", limiterId, expected })
    },
  }
